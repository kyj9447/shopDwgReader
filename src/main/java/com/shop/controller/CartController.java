...
public class CartController {
    ...
    public @ResponseBody ResponseEntity order(@RequestBody @Valid CartItemDto cartItemDto, BindingResult bindingResult, Principal principal) {
        ...
        Long cartItemId;
        try {
            cartItemId = cartService.addCart(cartItemDto, userInfo[0], userInfo[1]);
        } catch (Exception e) {
    ...
    public @ResponseBody ResponseEntity deleteCartItem(@PathVariable("cartItemId") Long cartItemId, Principal principal) {
        ...
        if (!cartService.validateCartItem(cartItemId, userInfo[0], userInfo[1])) {
    ...
    public @ResponseBody ResponseEntity orderCartItem(@RequestBody CartOrderDto cartOrderDto, Principal principal) {
        ...
        for (CartOrderDto cartOrder : cartOrderDtoList) {
            if (!cartService.validateCartItem(cartOrder.getCartItemId(), userInfo[0], userInfo[1])) {
                return new ResponseEntity<String>("주문 권한이 없습니다.", HttpStatus.FORBIDDEN);
            }
        }
        Long orderId = cartService.orderCartItem(cartOrderDtoList, userInfo[0], userInfo[1]);
